...
const MenuItemComponent = ({ item }: { item: MenuItem }) => {
  ...
  return (
    <li className="relative group no-last-gap">
      <Link
        href={href}
        className={`font-semibold text-white hover:text-lime-500 uppercase transition-all text-md py-[19px]`}
      >
        {item.label}
        {hasChildItems(item) && (
          ...
          {item.childItems.nodes.map((childItem: MenuItem) => (
            <li key={childItem.id}>
              <Link
                ...
                className={`block text-white hover:text-lime-500 text-sm py-2 px-3 border-b-[1px] border-slate-200 uppercase font-semibold`}
              >
                {childItem.label}
              </Link>
            </li>
